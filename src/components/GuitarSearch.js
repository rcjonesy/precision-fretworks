...
export const GuitarSearch = ({ newOrder, setNewOrder, setResults, results }) => {
    ...
    return (
        <>
            <input
                className="appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline bg-gray-200 placeholder-gray-500 mb-2"
                required
                type="text"
                ...
                <div className="bg-gray-200 text-black  overflow-y-auto max-h-96">
                    {
                        results?.map((result, index) => {
                            return (
                                ...
                                <div
                                    className={`relative cursor-pointer ${index === guitarIndex ? 'bg-gray-400 w-full' : 'hover:bg-gray-400'}`}
